...
/*
class Player {
	constructor(name, x, y, health, currentWeapon, isDefending) {
	this.name = name;
	this.x = x;
	this.y = y;
	this.health = health;
...
var player1 = new Player("player-1", 0, 0, 100, quarter, false);
var player2 = new Player("player-2", 0, 0, 100, quarter, false);*/
...
/*function playerTrap(){
	getPlayerPosition();
	const blockOneX = Math.abs(Number(keyblack.position.x) - Number(player1.position.x));
	const blockOneY = Math.abs(Number(keyblack.position.y) - Number(player1.position.y));
	const blockTwoX = Math.abs(Number(keyblack.position.x) - Number(player2.position.x));
	const blockTwoY = Math.abs(Number(keyblack.position.y) - Number(player2.position.y));
		return (((blockOneX == 0) && (blockOneY == 1)) ||
		((blockOneY == 0) && (blockOneX == 1))	||
		((blockTwoX == 0) && (blockTwoY == 1))||
		((blockTwoY == 0) && (blockTwoX == 1))
	)
...
//playerReset(player2);
	...
	/*else if(movePlayer(player1) || movePlayer(player2)){
		fightMode = false;
	}
	//getPlayerPosition();
	/*else if(getPlayerPosition() !== playerEncounter){
		fightMode = false;
		}*/
